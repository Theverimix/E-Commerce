...
export default function OrderPage() {
    ...
    const filteredOrders = orders.filter(order => {
        if (dateFilter === 'Last year') return isWithinLastYear(order.date);
        else if (dateFilter === 'Last 30 days') return isWithinLast30Days(order.date);
        else if (dateFilter === 'Last 7 days') return isWithinLast7Days(order.date);
        else return true;
    });
